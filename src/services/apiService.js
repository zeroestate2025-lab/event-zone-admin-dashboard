...
export const createBusinessPartner = async (formData) => {
  try {
    console.log('Creating business partner...');
    const response = await axios.post(`${API_BASE_URL}/business-partner/`, formData, {
      headers: {
        ...authHeader(),
      },
    });
    console.log('Business partner created successfully:', response.data);
    return response.data;
  } catch (error) {
    console.error("Error creating business partner:", error.response?.data || error.message);
    
    let errorMessage = 'Failed to create business partner';
    
    if (error.response?.data?.message) {
      errorMessage = error.response.data.message;
    } else if (error.response?.status === 502) {
      errorMessage = 'Server error (502 Bad Gateway)';
...
export const addPromotion = async (promotionData) => {
  try {
    console.log('Adding promotion with data:', promotionData);
...
export const updatePromotionById = async (promotionId, promotionData) => {
  try {
    console.log(`Updating promotion ${promotionId} with data:`, promotionData);
    
    // Don't send imageUrl in update - only send what's required
    const payload = {
      id: Number(promotionId),
      businessId: Number(promotionData.businessId),
      position: String(promotionData.position),
      isApproved: Boolean(promotionData.isApproved)
      // Remove imageUrl - it might be causing the issue
    };
    ...
    console.log(`Promotion ${promotionId} updated successfully:`, response.data);
    return response.data;
  } catch (error) {
...
//       isApproved: promotionData.isApproved !== undefined ? promotionData.isApproved : true,
...
//     return response.data;
//   } catch (error) {
//     console.error(`Error updating promotion ${promotionId}:`, error.response?.data || error.message);
...
export const deletePromotionById = async (promotionId) => {
  try {
    ...
    console.log(`Promotion ${promotionId} deleted successfully:`, response.data);
    return response.data;
  } catch (error) {
...
export const getPaymentByUserId = async (userId) => {
  try {
    const response = await axios.get(`${API_BASE_URL}/payment/user/${userId}`, {
      headers: authHeader(),
    });
    return response.data;
  } catch (error) {
    console.error(`Error fetching payment for user ${userId}:`, error);
