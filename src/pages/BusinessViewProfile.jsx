...
// Service and Subcategory Options
const SERVICE_OPTIONS = [
  "All", "Wedding", "Reception", "Birthday",
...
function BusinessViewProfile({ isSidebarOpen }) {
  ...
  useEffect(() => {
    const fetchBusinessData = async () => {
      ...
      try {
        ...
        const business = allPartners.find((b) => b.id === parseInt(businessId));

        if (!business) {
          setError(`No business found with ID ${businessId}`);
          setIsLoading(false);
          return;
        }
        ...
        const loadedFormData = {
          ...
          serviceProvided: business.serviceProvided || '',
          ...
          moreDetails: business.moreDetails || '',
          isApproved: business.isApproved || false,
          subCategories: business.subCategories || [],
          aproxLatitude: business.aproxLatitude || '',
          aproxLongitude: business.aproxLongitude || '',
        ...
        if (business.moreDetails) {
          try {
            const parsedDetails = JSON.parse(business.moreDetails);
            if (Array.isArray(parsedDetails) && parsedDetails.length > 0) {
              setCustomDetails(parsedDetails);
            }
          } catch (e) {
            console.error('Error parsing moreDetails:', e);
          }
        }
  ...
  const handleApprovalChange = async (e) => {
    const newApprovalStatus = e.target.checked;
    ...
    try {
      ...
      const moreDetailsString = JSON.stringify(
        customDetails.filter((d) => d.name && d.detail)
      );
      ...
      setSuccessMessage(
        `Profile ${newApprovalStatus ? 'approved and now visible' : 'hidden from'} in the application!`
      );
  ...
  const handleSubCategoryToggle = (category) => {
    setFormData((prev) => {
      ...
      const isSelected = currentSubCategories.includes(category);
      
      if (isSelected) {
        // Remove the category
        return {
          ...prev,
          subCategories: currentSubCategories.filter(cat => cat !== category)
        };
      } else {
        // Add the category
        return {
  ...
  const addCustomDetail = () => {
    setCustomDetails([...customDetails, { name: '', detail: '' }]);
  };

  const removeCustomDetail = (index) => {
    const newDetails = customDetails.filter((_, i) => i !== index);
    setCustomDetails(
      newDetails.length > 0 ? newDetails : [{ name: '', detail: '' }]
    );
  };
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const uploadedImage = await uploadBusinessImage(file, businessId);
  ...
  const navigateImage = (direction) => {
    ...
    setSelectedImageUrl(images[newIndex].url);
  };
  ...
  const handleEditToggle = () => {
    if (isEditing) {
      ...
      setCustomDetails(
        initialFormData.moreDetails
          ? JSON.parse(initialFormData.moreDetails)
          : [{ name: '', detail: '' }]
      );
      setError(null);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedFormData = { ...formData, moreDetails: moreDetailsString };
    ...
    return (
      ...
      >
        <div style={{ textAlign: 'center', padding: '60px 20px' }}>
          <FaSpinner
            style={{ fontSize: '48px', color: '#FC9256', animation: 'spin 1s linear infinite' }}
          />
  ...
  if (!businessId || isNaN(parseInt(businessId)) || error) {
    return (
      <div
        className={`profile-container ${
          isSidebarOpen ? 'sidebar-open' : 'sidebar-closed'
      ...
      >
        <div style={{ textAlign: 'center', padding: '60px 20px' }}>
          ...
          <button
            ...
            className="header-action-button back-button-header"
          >
            Go Back
          </button>
        </div>
  ...
  return (
    ...
    >
      <div className="profile-view-header">
        ...
        <h1 className="header-main-title">Business Partner Profile</h1>
        <button
          onClick={handleEditToggle}
          className="header-action-button edit-button-header"
      ...
      <div className="approval-visibility-section">
        <div className="approval-checkbox-row">
          <label className="approval-checkbox-label">
            <input
              ...
              disabled={isSubmitting}
              className="approval-checkbox-input"
            />
            <span className="approval-checkbox-text">
              Approve this profile to display in the application
            </span>
            <span className={`approval-status-indicator ${formData.isApproved ? 'approved' : 'pending'}`}>
              {formData.isApproved ? 'Visible in Application' : 'Hidden from Application'}
            </span>
          </label>
        </div>
      </div>
        ...
        <div className="info-card-grid">
          ...
          <div className="info-card">
            ...
            {isEditing ? (
              ...
              <span>{formData.proprietorName}</span>
            )}
          </div>
          ...
          <div className="info-card">
            ...
            {isEditing ? (
              ...
              <span>{formData.serviceProvided}</span>
            )}
          </div>
        ...
        <div className="edit-section-container">
            ...
            <div className="edit-form-field">
              ...
              {isEditing ? (
                <input
                  type="text"
                  name="aproxLatitude"
                  value={formData.aproxLatitude}
                  onChange={handleInputChange}
                />
              ) : (
                <span>{formData.aproxLatitude}</span>
              )}
            </div>
            ...
            <div className="edit-form-field">
              ...
              {isEditing ? (
                <input
                  type="text"
                  name="aproxLongitude"
                  value={formData.aproxLongitude}
                  onChange={handleInputChange}
                />
              ) : (
                <span>{formData.aproxLongitude}</span>
              )}
            </div>
            ...
            <div className="edit-form-field full-width">
              ...
              {isEditing ? (
                <div className="subcategory-checkbox-container">
                  {SUB_CATEGORY_OPTIONS.map((category) => (
                ...
                <span>{(formData.subCategories || []).join(', ')}</span>
              )}
            </div>
          </div>

          <div className="edit-form-details-section">
            <div className="details-label-with-warning">
              <label>More Details:</label>
              <div className="warning-indicator">
                <FaExclamationTriangle className="warning-icon" />
                <span className="warning-text">Please mention the Google map Link as "gmap" exactly</span>
              </div>
            </div>
            {isEditing ? (
                ...
                {customDetails.map((detail, index) => (
                  <div key={index} className="custom-detail-editor-row">
                    <input
                      ...
                      value={detail.name}
                      onChange={(e) =>
                        handleCustomDetailChange(index, 'name', e.target.value)
                      }
                    />
                    <input
                      type="text"
                      ...
                      value={detail.detail}
                      onChange={(e) =>
                        handleCustomDetailChange(index, 'detail', e.target.value)
                      }
                    />
                    <button
                      type="button"
          ...
          <div className="image-upload-section">
            ...
            <div className="upload-activate-row">
              <label
                htmlFor="imageUpload"
                className={`upload-image-button ${imageUploading ? 'disabled' : ''}`}
              >
                {imageUploading ? (
                  <>
...
//             <strong>Note:</strong> <span className="note-checked">Checked</span> = Profile is visible to all users in the mobile application. 
//             <span className="note-unchecked">Unchecked</span> = Profile is hidden and will not be displayed.
//           </p>
//         </div> */}
//       </div>
...
// //           <div className="edit-form-details-section">
// //             <label>More Details: Please mention the Google map Link has "gmap" exactly</label>
// //             {isEditing ? (
// //               <div className="more-details-editor">
// //                 {customDetails.map((detail, index) => (
// //                   <div key={index} className="custom-detail-editor-row">
