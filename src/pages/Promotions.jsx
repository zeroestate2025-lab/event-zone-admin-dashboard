...
function Promotions({ isSidebarOpen }) {
  ...
  const handleOpenPromoModal = (promotion = null) => {
    if (promotion) {
      ...
      setSelectedBusinessId(promotion.businessId?.toString() || "");
      setSelectedPosition(promotion.position?.toString() || "");
      setSelectedImage(promotion.imageUrl || "");
      setIsApproved(promotion.isApproved !== undefined ? promotion.isApproved : true);
    } else {
  ...
  const handleSubmitPromotion = async (e) => {
    ...
    try {
      ...
      if (editingPromotion) {
        ...
        const response = await updatePromotionById(editingPromotion.id, payload);
      ...
      if (err.response?.data?.message) {
        ...
        errorMessage = 'Server error (502). Please try again.';
      } else if (err.message) {
              ...
              return (
                <div key={promo.id} className="promotion-card">
                  <div className="promotion-image-container">
                    ...
                    {promo.isApproved && (
                      <div className="promotion-approved-badge">
                        ✓ Approved
                      </div>
                    )}
                  </div>
            ...
            <form onSubmit={handleSubmitPromotion}>
                    ...
                    {getBusinessById(parseInt(selectedBusinessId))?.images?.map(
                      (img) => (
                        <div
                          ...
                          onClick={() => !isPromoSubmitting && setSelectedImage(img.url)}
                        >
              ...
              <div className="modal-form-group">
                ...
                <input
                  type="text"
                  id="position-input"
                  placeholder="Enter position (e.g., top, 1, 2, 3...)"
                ...
                <small className="input-hint">
                  Position determines order of display (e.g., "top", "1", "2", etc.)
                </small>
              </div>
